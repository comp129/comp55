...
This is because `ArrayList`, like other collections in Java, is designed to work with objects, not primitives. 
To store primitive values in an `ArrayList`, Java provides wrapper classes (e.g., `Integer` for `int`, `Double` for `double`) that 
encapsulate primitive values as objects. Java's autoboxing feature automatically converts a primitive value to its corresponding 
wrapper class when added to an `ArrayList`. Conversely, unboxing automatically extracts the primitive value from its wrapper when retrieved.

For example, to store integers in an `ArrayList`, you would declare it as `ArrayList<Integer>`, and Java takes care of 
converting `int` values to `Integer` objects upon addition to the list. This seamless conversion between primitives 
and their corresponding wrapper classes allows for the flexible use of collections with primitive types.
...
public class AutoboxingExample {
    public static void main(String[] args) {
        ...
        // Accessing the Integer object
        Integer retrievedNumber = list.get(0);

        // Unboxing: The Integer object is automatically converted back to a primitive int
        int primitiveNumber = retrievedNumber;
...
For a more in-depth understanding of autoboxing and unboxing in Java, refer to this comprehensive guide: [Autoboxing and Unboxing](https://docs.oracle.com/javase/tutorial/java/data/autoboxing.html).
