...
- [Step 5. Adding GImages](#step-5-adding-gimages)
    - [First Parameter - Filename](#first-parameter---filename)
    - [Second/Third Parameters - X and Y Coordinates](#secondthird-parameters---x-and-y-coordinates)
- [Step 6: Applying Color](#step-6-applying-color)
    - [Overlapping Objects](#overlapping-objects)
- [Step 7: Make a robot (Upload your robot picture as your deliverable)](#step-7-make-a-robot-upload-your-robot-picture-as-your-deliverable)
...
- Currently, there is only one call on line 16, which is,
  ...
  you then need to add them to your canvas.
    ...
    - What's inside the ```add``` command ```new GLabel("Hello World", 300, 300)```
      can be thought of as making a shape (or label in our case).
...
1. **Store the new ```GLabel``` into a variable called ```label```.**
    ...
    - Once you have the ```GLabel``` in a variable, rather than adding it right away,
    ...
    > it would make sense to store that ```GLabel``` into a variable that we could then refer to.

2. **Go ahead and color the label ```red```**

    - Add ```label.setColor(Color.red);``` in between creating the label and adding it to the screen.

3. **add ```label.setFont("Arial-Bold-24")```**

    > The font just needs to be a font that is registered on your computer,
    > followed by the style and size, separated by hyphens.
    > The style is optional (and will be left out in examples),
...
- *From here on out I will refer to those as ```START_X``` and ```START_Y```.*
    ...
    > it makes sense to create final variables, which are constants in Java.
    ...
    > There, to denote that it's a constant and that it represents the starting X point for the code.
...
> - In the rectangle, `300, 300` is the ***upper left of the rectangle***,
> - With GLabels, `300, 300` is the ***lower left***.

### Ovals

1. **To understand how the `GOval` works, let's go ahead and add an oval
   with the same arguments as the rectangle.**

    - ```run``` should look something like what's below… ***but hopefully with more constants!***

    > ```GOval``` works by creating the largest oval that can fit inside a rectangle
    > with the given dimensions *(A circumscribed oval)*.
...
- In addition to this, ```GOval``` and ```GRect``` can be filled,
...
> The best way to learn is to just *play around with the arguments* for each shape
...
- However, since we are loading something other than code,
...
### Second/Third Parameters - X and Y Coordinates

- You may retrieve the size of any image object after creation with ```getWidth()``` or ```getHeight()```
- You can use this in combination with a ```setLocation(x, y)``` that will change the object's location

There are different types of Images that can be imported.
Some include:
...
> When referenecing image paths, you can copy them from you Resources folder. With the image selected pres Alt and Enter and selcted the path manually. As with simple Copy and Paste you wont get the full path.
...
Here is a table showing you how to apply color to two such shapes: `GLabel` and `GRect`.
...
Remember that when working with colors you should:

1. Create the Fillable `GObject` and store it in a temporary variable
2. Use ```.setFilled(true)```
3. Use ```.setFillColor(Color.INSERT_COLOR_HERE)```
   > This sets the fill Color of the shape you have made
4. Use ```add()``` to get it on the screen

### Overlapping Objects

Making objects overlap can give you some interesting shapes or designs.
The best way to control this is by ***changing the order in which you add the objects.***

> As you become more proficient you may want certain shapes
> to appear in front of others in case they are overlapping.
...
## Step 7: Make a robot (Upload your robot picture as your deliverable)
...
> For those of you that are curious, this is not a gradient at all,
...
> they are expressed as a combination of red, green and blue integers between 0 and 255.
>
...
> or vice-versa, just like you did in C++.
...
However, if you feel like you have a robot that is awesome,
rather than leaving, you should begin to explore some animation.
...
> followed by a ```pause``` of 10 milliseconds, and see the magic happen.
...
You can find out more about those and other items in the graphics library by visiting:
